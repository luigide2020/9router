...
try:
    ...
    print("❌ 运行: uv add playwright && uv run playwright install chromium"); sys.exit(1)
...
def do_login(page, email, password):
    print("[LOGIN] 输入邮箱...")
    page.fill('input[type="email"]', email); page.click('input[type="submit"]')
    print("[LOGIN] 等待密码框...")
    page.wait_for_selector('input[type="password"]', timeout=15000)
    page.fill('input[type="password"]', password); page.click('input[type="submit"]')
    try:
        page.wait_for_selector('#idSIButton9', timeout=8000); page.click('#idSIButton9')
    except PwTimeout:
        pass
...
def main():
    ...
    if not args.sniff_only and (not email or not password):
        print("[ERROR] 请设置 M365_EMAIL 和 M365_PASSWORD"); sys.exit(1)
    ...
    def save_target(token, source_url):
        ...
        exp = c.get("exp")
        data = {
            "accessToken": token, "source": "ws-chathub",
            ...
            "aud": c.get("aud", "unknown"), "scp": c.get("scp", "unknown"),
        ...
        if exp: print(f"  剩余: {(exp - time.time())/60:.0f} 分钟")

    def capture_from_url(url):
        if CHATHUB_PATH not in url.lower(): return False
    ...
    def on_ws(ws):
        ...
        def scan(payload):
            if target["token"] is not None: return
            if CHATHUB_PATH not in ws.url.lower(): return
            try:
    ...
    def type_one_char(page):
        for sel in ['div[contenteditable="true"]', 'textarea', '[role="textbox"]']:
            try:
                ...
                try: box.press("Control+A"); box.press("Delete")
                ...
                continue
        print("[WARN] 没定位到输入框"); return False
    ...
    with sync_playwright() as p:
        ...
        if not args.sniff_only:
            if is_logged_in(page): print("[LOGIN] ✅ 已有登录态")
            else: do_login(page, email, password)

        page.wait_for_url("**/chat**", timeout=30000)
        page.wait_for_timeout(3000)

        for i in range(1, args.attempts + 1):
            if target["token"]: break
            print(f"\n========== 第 {i}/{args.attempts} 轮:reload → 敲字 → 等 WS ==========")
            try: page.reload(wait_until="domcontentloaded")
            except Exception as e: print(f"[WARN] reload 失败: {e}")
            ...
            if target["token"]:
                print(f"[INFO] 第 {i} 轮成功抓到 token"); break
        ...
        if args.close and target["token"]:
            ...
            print("\n浏览器保持打开。退出按 Ctrl+C。")
            try:
                while True: page.wait_for_timeout(1000)
